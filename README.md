# noticeboard-v2
전에 쓴 게시판 코드중 만들고 Stringrequest클래스를 사용해서 구현한것을 이미만들어봤기때문에 코드는 그대로이고 직접 만든 생성자 사용을 위해 construct_stringrequest클래스를 만들고 사용해서 코드는 다르지만 같은 프로그램을 구현하는 단계이기때문에 이 레포지토리에 구동하는사진은 안만들고 construct_stringrequest클래스 코드만 올리겠습니다. 현재 진행중
