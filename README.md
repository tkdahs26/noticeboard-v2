# noticeboard-v2
전에 쓴 게시판 코드중 만들고 Stringrequest클래스를 반복 사용해서 구현한것을 이미만들어봤기때문에 코드는 그대로이고 재사용성이있는  construct_streingrequest클래스파일을만들고 따로 구현하는 최적화 단계이기때문에 이 레포지토리에 구동하는사진은 안만들고 construct_streingrequest클래스 코드만 올리겠습니다. 현재 진행중
